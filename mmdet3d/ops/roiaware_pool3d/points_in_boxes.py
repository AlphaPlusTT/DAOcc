...
def points_in_boxes_aux_cpu(points, boxes):
    """Find points that are in boxes (CPU)
    ...
    point_indices = points.new_zeros((boxes.shape[0], points.shape[0]), dtype=torch.int)
    point_regs = points.new_zeros((points.shape[0], 3), dtype=torch.float)
    roiaware_pool3d_ext.points_in_boxes_aux_cpu(
        boxes.float().contiguous(), points.float().contiguous(), point_indices, point_regs
    )

    return point_indices, point_regs
...
def points_in_boxes_aux_batch(points, boxes):
    """Find points that are in boxes (CUDA)
    ...
    box_center_reg_of_pts = points.new_zeros((batch_size, num_points, 3), dtype=torch.float).fill_(0)
    ...
    roiaware_pool3d_ext.points_in_boxes_aux_batch(
        boxes.contiguous(), points.contiguous(), box_idxs_of_pts, box_center_reg_of_pts
    )

    return box_idxs_of_pts, box_center_reg_of_pts
