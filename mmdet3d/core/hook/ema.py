...
class MEGVIIEMAHook(Hook):
    ...
    def save_checkpoint(self, runner):
        ...
        # TODO: check if this function is already implemented in mmcv!
        # check and delete old ckpt, keeping at most 'max_keep_ckpt'
        ckpt_files = glob(os.path.join(runner.work_dir, 'epoch_*_ema.pth'))
        ckpt_files.sort(key=lambda x: int(x.split('/')[-1].split('_')[1]))  # 按epoch排序
